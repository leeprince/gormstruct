...
func (g *GenDBInfo) genTableElement(els []ColumnsElementInfo) (genEls []genstruct.GenElement) {
	var columnNameMap = make(map[string]bool, len(els))

	for _, el := range els {
		var genEl genstruct.GenElement
		columnName := utils.GetCamelName(el.Name)
		for i := 2; i < 100; i++ { // 最多计算100次
			if i >= 99 {
				panic("表字段名重复次数超过99次，字段：" + columnName)
			}

			if _, ok := columnNameMap[columnName]; ok {
				columnName = fmt.Sprintf("%s%s%d", columnName, "Gorm", i)
			} else {
				columnNameMap[columnName] = true
				break
			}
		}
...
func (g *GenDBInfo) generateFunc() (genOut []GenOutInfo) {
	...
	// 根据表的属性信息构建模版所需字段
	var columnNameMap = make(map[string]bool, len(g.info.Table.ColumnsElement))
